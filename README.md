...
- A Vercel account
- A Pollinations API key for image and video generation from [enter.pollinations.ai](https://enter.pollinations.ai)
...
6. Redeploy the project so Vercel picks up the new key.

## Important notes

- `MONGODB_URI` is optional for the UI to load, but required if you want requests saved to MongoDB.
- `POLLINATIONS_API_KEY` is required. If it is missing, the app will return a setup message instead of generating media.
